...
{
	spdm_state->transcript = vmalloc(PAGE_SIZE);
			   ...
			   const void *msg, size_t msg_sz)
{
	...
	if (transcript_sz + msg_sz > spdm_state->transcript_max) {
		...
		void *new = vrealloc(spdm_state->transcript, new_sz,
				     GFP_KERNEL);
		if (!new)
			return -ENOMEM;
			  ...
			  const char *spdm_context)
{
	struct public_key_signature sig = {
		...
		.hash_algo = hash_algo_name[spdm_state->base_hash_alg],
	};
	...
	m = kmalloc(SPDM_COMBINED_PREFIX_SZ + hash_len, GFP_KERNEL);
	if (!m)
		return -ENOMEM;
	...
	if (spdm_state->version <= 0x11) {
		...
		default:
			mhash = kmalloc(hash_len, GFP_KERNEL);
			if (!mhash)
				return -ENOMEM;

			rc = crypto_shash_digest(spdm_state->desc, m,
				SPDM_COMBINED_PREFIX_SZ + hash_len, mhash);
			if (rc)
				return rc;
