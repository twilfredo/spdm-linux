...
static int spdm_parse_algs(struct spdm_state *spdm_state, u32 base_asym_alg,
			   u32 base_hash_alg)
{
	spdm_state->base_asym_alg = base_asym_alg;

	switch (base_asym_alg) {
	...
	switch (base_hash_alg) {
	case SPDM_HASH_SHA_256:
		spdm_state->base_hash_alg = HASH_ALGO_SHA256;
		break;
	case SPDM_HASH_SHA_384:
		spdm_state->base_hash_alg = HASH_ALGO_SHA384;
		break;
	case SPDM_HASH_SHA_512:
		spdm_state->base_hash_alg = HASH_ALGO_SHA512;
		break;
	default:
	 ...
	 */
	spdm_state->shash =
		crypto_alloc_shash(hash_algo_name[spdm_state->base_hash_alg],
				   0, 0);
	if (IS_ERR(spdm_state->shash))
		return PTR_ERR(spdm_state->shash);
...
static int spdm_negotiate_algs(struct spdm_state *spdm_state)
{
	...
	u32 base_asym_alg, base_hash_alg;
	...
	*req = (struct spdm_negotiate_algs_req) {
	...
	base_asym_alg = le32_to_cpu(rsp->base_asym_sel);
	base_hash_alg = le32_to_cpu(rsp->base_hash_sel);

	if ((base_asym_alg & SPDM_ASYM_ALGOS) == 0 ||
	    (base_hash_alg & SPDM_HASH_ALGOS) == 0) {
		dev_err(spdm_state->dev, "No common supported algorithms\n");
		return -EPROTO;
	}

	/* Responder shall select exactly 1 alg (SPDM 1.0.0 table 14) */
	if (hweight32(base_asym_alg) != 1 ||
	    hweight32(base_hash_alg) != 1 ||
	...
	return spdm_parse_algs(spdm_state, base_asym_alg, base_hash_alg);
}

static int spdm_get_digests(struct spdm_state *spdm_state)
{
	struct spdm_get_digests_req *req = spdm_state->transcript_end;
	int hash_len = hash_digest_size[spdm_state->base_hash_alg];
	...
	*req = (struct spdm_get_digests_req) {
	 ...
	 */
	rsp_sz = sizeof(*rsp) + SPDM_SLOTS * hash_len;
	...
	length = rc;
	if (length < sizeof(*rsp) ||
	    length < sizeof(*rsp) + hweight8(rsp->param2) * hash_len) {
		dev_err(spdm_state->dev, "Truncated digests response\n");
		return -EIO;
	}

	spdm_state->transcript_end +=
		     sizeof(*rsp) + hweight8(rsp->param2) * hash_len;
...
static int spdm_get_certificate(struct spdm_state *spdm_state, u8 slot)
{
	...
	header_length = sizeof(struct spdm_cert_chain) +
...
static int spdm_challenge(struct spdm_state *spdm_state, u8 slot)
{
	...
	spdm_netlink_sig_event(spdm_state, SPDM_CHALLENGE_AUTH, slot,
			       req_nonce_off, rsp_nonce_off, spdm_context);
