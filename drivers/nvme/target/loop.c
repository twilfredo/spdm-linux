...
static void nvme_loop_destroy_admin_queue(struct nvme_loop_ctrl *ctrl)
{
	...
	WARN_ON(nvmet_cq_destroy(&ctrl->queues[0].nvme_cq) != NVME_SC_SUCCESS);
...
static void nvme_loop_destroy_io_queues(struct nvme_loop_ctrl *ctrl)
{
	...
	for (i = 1; i < ctrl->ctrl.queue_count; i++) {
		...
		WARN_ON(nvmet_cq_destroy(&ctrl->queues[i].nvme_cq) != NVME_SC_SUCCESS);
	}
...
static int nvme_loop_init_io_queues(struct nvme_loop_ctrl *ctrl)
{
	...
	for (i = 1; i <= nr_io_queues; i++) {
		...
		if (ret) {
			/*
			 * Destroy the latest CQ initialized, rest will be cleaned up
			 * by nvme_loop_destroy_io_queues() depending on`queue_count`.
			 */
			nvmet_cq_destroy(&ctrl->queues[i].nvme_cq);
