...
int nvmet_cq_get(struct nvmet_cq *cq)
{
...
u16 nvmet_check_cmd_cqid(struct nvmet_ctrl *ctrl, u16 cqid, bool create)
{
	if (!cqid)
		return NVME_SC_QID_INVALID | NVME_STATUS_DNR;
...
bool nvmet_cq_is_deletable(struct nvmet_cq *cq)
{
	if (refcount_read(&cq->ref) > 1)
		return true;

	return false;
}
EXPORT_SYMBOL_GPL(nvmet_cq_is_deletable);
