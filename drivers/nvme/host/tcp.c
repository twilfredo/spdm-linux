		...
		struct nvme_completion *cqe)
{
	...
	trace_printk("CQE: qid=%d cid=%d, status=0x%x, sq_head=%d\\n",
		     nvme_tcp_queue_id(queue), cqe->command_id,
		     le16_to_cpu(cqe->status), le16_to_cpu(cqe->sq_head));
...
static int nvme_tcp_try_send(struct nvme_tcp_queue *queue)
{
	...
	/*
	 * Pending TLS partial rercords should be sent before we
	 * try to enqueue more records. write_space() will kick off
	 * this process.
	 */
	if (nvme_tcp_queue_tls(queue) &&
	    tls_is_partially_sent_record(ctx))
