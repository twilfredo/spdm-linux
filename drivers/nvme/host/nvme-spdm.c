...
void nvme_spdm_init(struct device *dev)
{
	...
	/*
	 * Use a work queue for authentication work, so that we don't run
	 * into issues with loading modules with a wait from an async context.
	 */
	nvme_spdm_wq =  alloc_workqueue("nvme-spdm-wq", WQ_MEM_RECLAIM, 0);
	if (!nvme_spdm_wq)
		return;
 ...
 * using the SPDM over NVMe Storage transport. This creates the respective SPDM
 * related sysfs attributes without complaining if they already exist.
 */
